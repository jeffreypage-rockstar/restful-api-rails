class AddCiLowerBoundFunction < ActiveRecord::Migration
  def up
    execute <<-SQL
      create or replace function ci_lower_bound(ups integer, downs integer)
      returns numeric as $$
    ...
    execute "CREATE INDEX index_comments_rank ON comments "\
            "((ci_lower_bound(up_score, down_score)));"
  end
